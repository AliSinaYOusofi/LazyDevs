...
export default function AccountDetails() {
    ...
    const [currentUser, setCurrentUserData] = useState(undefined)
    const [retryUserFetch, setRetryUserFetch] = useState(false)
    const [error, setError] = useState(false)

    const fetchNewDataForUser = useCallback( async () => {
        try {
            const response = await fetch(`http://localhost:3001/accountRoutes/my_updated_info`, 
                {
                    method: "GET",
                    credentials: "include"
                }
            ...
            <div className="w-full h-full flex items-center justify-center">
                {
                    ...
                    <button 
                        ...
                        onClick={() => setRetryUserFetch( prev => ! prev)}
                    >
                        <svg
                            viewBox="0 0 512 512"
                            fill="currentColor"
    ...
    return (
        ...
        <div className="md:px-6 flex flex-col items-center justify-evenly  bg-white p-12">
